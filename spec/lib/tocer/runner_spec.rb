...
RSpec.describe Tocer::Runner, :temp_dir do
  ...
  let(:markdown_file) { Pathname.new File.join(temp_dir, "test.md") }
  let(:text_file) { Pathname.new File.join(temp_dir, "test.txt") }
  ...
  describe "#files" do
    ...
    context "with path only" do
      ...
      it "answers all files in path" do
        expect(subject.files).to contain_exactly(markdown_file, text_file)
      end
    end
    ...
    context "with path and whitelist string" do
      ...
      it "answers files with matching extensions only" do
